...
class Model(nn.Module):
    ...
    def forward_once(self, x, x2, profile=False):
        y, dt = [], []  # outputs
        for m in self.model:
            # Xác định đầu vào cho layer
            if m.f != -1:
                if m.f == -4:
                    x = x2  # Lấy từ luồng ảnh thứ 2 (IR)
                else:
                    # Lấy từ tầng trước hoặc gộp từ nhiều tầng (list)
                    x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]

            # Chạy module (không còn try-except và print)
            x = m(x)

            # Lưu output nếu layer nằm trong danh sách cần save
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # --- PHẦN LOGIC ĐÃ ĐƯỢC FIX ---
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3, C3TR, MCPSFusion, CAANNeck, RFDNNeck, GLCIPool, C2f]:
        
            
            # 1. Xác định c1 (Đầu vào)
            if m is Focus:
                c1 = 3 # Focus luôn nhận ảnh 3 kênh (RGB hoặc IR)
            elif isinstance(f, int):
                c1 = ch[f]
            else:
                c1 = sum([ch[x] for x in f if ch[x] is not None])

            # 2. Xác định c2 (Đầu ra)
            c2 = args[0]
            if c2 != no:  # width gain
                c2 = make_divisible(c2 * gw, 8)

            # 3. Đóng gói args cho module
            if m is Focus:
                args = [c1, c2, *args[1:]]
            elif m is C2f:
                # args ban đầu từ YAML là [c2, shortcut]
                # Chúng ta chèn n vào giữa để thành [c1, c2, n, shortcut]
                args = [c1, c2, n, *args[1:]]  
                n = 1      # Reset n về 1 để nn.Sequential không nhân bản khối C2f thêm lần nữa
            elif m is GLCIPool:
                c1, c2 = ch[f], args[0]
                # GLCIPool nén c1 xuống c_ = c1/2, sau đó cat 5 nhánh nên c_out = (c1/2) * 5
                # Tuy nhiên, để đồng bộ với YOLOv5, ta ép c_out về lại c2 (thường là 512 hoặc 1024)
                args = [c1, c2, *args[1:]]
            elif m is MCPSFusion:
                # MCPSFusion cần channels gốc của 2 luồng
                c1_vis = ch[f[0]]
                c2_ir = ch[f[1]]
                args = [c1_vis, c2_ir]
                c2 = c1_vis + c2_ir
            elif m in [CAANNeck, RFDNNeck]:
                args = [c1, c2]
            ...
            # Lọc bỏ các giá trị None hoặc chỉ số âm đặc biệt trước khi tính tổng kênh
            c2 = sum([ch[x if x == -1 else x] for x in f if ch[x] is not None])
        elif m in [Add, Add2, GPT]: # Gom các layer cộng/biến đổi đơn luồng
            c2 = ch[f[0]]
            args = [c2] if m is not Add2 else [c2, args[1]]
        elif m is Detect:
        ...
        if i == 24:
            print(f"--- PARSE LAYER 24: c1={c1}, c2={c2}, args={args} ---")

        # Khởi tạo Module
        m_ = nn.Sequential(*[m(*args) for _ in range(n)]) if n > 1 else m(*args)
        t = str(m)[8:-2].replace('__main__.', '')  # module type
        np = sum([x.numel() for x in m_.parameters()])  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach metadata
        logger.info('%3s%18s%3s%10.0f  %-40s%-30s' % (i, f, n, np, t, args))  # log
        save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x is not None and x != -1)
        layers.append(m_)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='C:\\Users\\Tai\\Desktop\\DAT_K22\\dataset\\drone\\multispectral-object-detection-main\models\\transformer\\model_baseline.yaml', help='model.yaml')
    ...
    input_rgb = torch.Tensor(1, 3, 320, 256).to(device)
    input_ir = torch.Tensor(1, 3, 320, 256).to(device)
