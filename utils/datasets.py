...
def img2label_paths(img_paths):
    # Định nghĩa lại để hỗ trợ cả images_rgb và images_ir
    # Bước 1: Thay thế 'images_rgb' hoặc 'images_ir' hoặc 'images' bằng 'labels'
    label_paths = []
    for x in img_paths:
        for tag in ['images_rgb', 'images_ir', 'images']:
            if tag in x:
                x = x.replace(tag, 'labels')
                break
        
        # Bước 2: Đổi đuôi file ảnh (jpg, png...) thành .txt
        suffix = x.split('.')[-1]
        lb_path = x.rsplit('.' + suffix, 1)[0] + '.txt'
