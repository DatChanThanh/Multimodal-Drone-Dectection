...
def test(data,
    ...
    if training:  # called by train.py
        ...
        try:
            ...
            # Tạo dữ liệu giả lập cho cả 2 luồng RGB và IR (size imgsz x imgsz)
            input_rgb = torch.randn(1, 3, imgsz, imgsz).to(device)
            input_ir = torch.randn(1, 3, imgsz, imgsz).to(device)
            
            # Đo lượng tính toán của model(img_rgb, img_ir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--plots', action='store_true', help='save plot images')
